...
namespace detail {
	struct encryptor {
		static void encrypt(char *dest, const char *str, size_t len, char key) {
...
public:
...
#endif
		string_encryptor(const char *str, size_t len) :
	...
	private:
		mutable char _buffer[ENCR_BUFFER_MAX_LENGTH+1];
		mutable bool _decrypted;
		const char _key;
		size_t _length;
	};

	static auto create(const char* str, size_t len) {
